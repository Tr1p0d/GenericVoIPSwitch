-module(generic_exchange_sip_transport).
		...
		{ok, RawSipMsg, _More} ->
			case nksip_parse:raw_sipmsg(RawSipMsg) of
				Msg=#sipmsg{} ->
					...
					% processing
					generic_exchange_sip_gateway:route_sip(Msg, IP, Port);
				{error, Code, _} -> 
...
terminate(_Reason, _State) ->
	lager:warning("shutting down the UDP transport because of ~p",[_Reason]),
	ok.
