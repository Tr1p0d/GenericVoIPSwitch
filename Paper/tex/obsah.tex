...
It represents a transaction and its and transitions which could be taken as a reaction various generic messages. When a signaling transaction is started, the exchange creates 2 of this finite state machines. 1 for every part of transaction ( in our case client and server transaction ). The reason why, there are 2 transactions is for cases where 1 transaction wont suffice such as when user calls itself.

\chapter{General VoIP Exchange design}
\begin{figure}[h!]
\centering
\includegraphics[scale=0.4]{fig/exchange_desing_1}
\caption[Generic exchange initial design] {Generic exchange initial design}
\end{figure}

\section{Protocol specific parts of exchange}



\begin{figure}[h!]
\centering
\includegraphics[scale=0.7]{fig/exchange_design_supervision_tree}
