...
\item gateway
\item gatekeeper
\item multipoint control unit ( MCU )
\end{itemize}
...
SIP protocol intelligence is distributed across different network devices. Every network device that is part of SIP signalization topology is called a User Agent ( UA ). UA are further divided into a quite complex hierarchy. A picture below serves as an topology example.
...
%\end{figure}
\begin{itemize}
\item User Agent Clients
\item User Agent Servers
	\begin{itemize}
	\item Server Proxy is responsible for signalization routing
	\item Server Registrar provides registration point for clients
%	\item Server Location 
%	\item Server Redirect 
	\end{itemize}
\end{itemize}
...
It is binary protocol with client-server, request-response architecture and only a few basic messages. Such a concept is then reflected in protocol messages. They are much more low-level. For example if there is a call for an end device, exchange wont leave incomming call handling on a device. Instead it will tell which port, should be opened for receiving payload, what codec will be in use and to start ringing.

\section{RTP}
Is an application layer protocol for real-time transfer of data streams. It is considered a standard for transporting voice or video media in packet-switched networks. It is one of key elements of VoIP. It wokrks in pair with ( Real-time Transport Control Protocol ) where RTP carries media streams and RTCP carries flow information, creates statistics and optimizes the RTP streams. Since VoIP works upon packet-switched network RTP includes mechanisms to compensate jitter, messages received out of sequence.
...
\subsection{Maps}
\lstinputlisting[style=customhaskell, caption=map function example in Haskell programming language]{scodes/map_example.hs}
\lstinputlisting[style=custompython, caption=similar program written in Python]{scodes/map_example.py}
\subsection{Filters}
\lstinputlisting[style=customerl, caption=filter function example written in Erlang]{scodes/filter_example.erl}
\lstinputlisting[style=customperl, caption=similar program written in Perl]{scodes/filter_example.pl}
\subsection{Folds}
...
When using a behaviour, programmer oblige to implement function call-backs in specific module. Such call-backs are then called from generic module.
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.7]{fig/event_handler_example}
...
\chapter{Designing VoIP Exchange core}
\section{What is an application core}
...
In terms of generic exchange, a core should be generic finite state machine. It is the most crucial part, theoretically shared by an arbitary number of specific protocol ( SIP, H.323, LCPv1). Since every protocol is different designing errorless, shared, generic logic is very difficult.
...
\chapter{General VoIP Exchange design}

\chapter{Conclusion}%Závìreèná kapitola obsahuje zhodnocení dosa¾ených výsledkù se zvlá¹» vyznaèeným vlastním pøínosem studenta. Povinnì se zde objeví i zhodnocení z pohledu dal¹ího vývoje projektu, student uvede námìty vycházející ze zku¹eností s øe¹eným projektem a uvede rovnì¾ návaznosti na právì dokonèené projekty.%=========================================================================
